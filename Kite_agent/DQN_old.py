...
class DeepQNetwork:
    def __init__(
        ...
        self.n_fig = n_input # figure size [n1, n2, n_channel]
    ...
    def _build_net(self):
        '''   
        # build CNN network 
        self.s  = tf.placeholder(tf.float32, [None, self.n_fig[0], self.n_fig[1], self.n_fig[2]], name='s')  # state, input for eval net
        self.s_ = tf.placeholder(tf.float32, [None, self.n_fig[0], self.n_fig[1], self.n_fig[2]], name='s_') # new state, input for target net
        ...
        self.s  = tf.placeholder(tf.float32, [None, self.n_fig], name='s')  # state, input for eval net
        self.s_ = tf.placeholder(tf.float32, [None, self.n_fig], name='s_') # new state, input for target net
        self.q_target = tf.placeholder(tf.float32, [None, self.n_actions], name='Q_target')  # for calculating loss
        self.q_eval = self.build_dense(self.s, 'eval_net_params')      
        self.q_next = self.build_dense(self.s_, 'target_net_params')   # [None, n_actions]
